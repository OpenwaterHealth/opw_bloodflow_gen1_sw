...
#if 0
#define HANDLE_ASSERT(expr_)
#else
#define HANDLE_ASSERT(expr_) assert((expr_))
#endif
...
T& CircularBuffer<T>::Next() {
  HANDLE_ASSERT(data_.size() + popped_ - pushed_ >= 1);
  return data_[head_];
}
...
T& CircularBuffer<T>::Pop() {
  HANDLE_ASSERT(pushed_ - popped_ >= 1);
  int tail = tail_;
...
T& CircularBuffer<T>::Peek(size_t n) {
  HANDLE_ASSERT(pushed_ - popped_ >= (n + 1));
