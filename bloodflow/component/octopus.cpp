...
int Octopus::NumOctopi() { return Driver::NumDevices(PID); }
...
void Octopus::ConfigureTimer(const Timer& ot) {
  ...
  for (int i = 1; i < 7; ++i) {
    assert(ot.state[i].period < (((uint64_t)1 << 32) - 1) / OCTO_CLK_FREQ);
  ...
  count = (uint32_t)(ot.state[1].period * OCTO_CLK_FREQ);
  regmap[2] = count & 0xFFFF;
  regmap[3] = count >> 16;
  count = (uint32_t)(ot.state[2].period * OCTO_CLK_FREQ);
  regmap[4] = count & 0xFFFF;
  regmap[5] = count >> 16;
  count = (uint32_t)(ot.state[3].period * OCTO_CLK_FREQ);
  ...
  count = (uint32_t)(ot.state[4].period * OCTO_CLK_FREQ);
  regmap[9] = count & 0xFFFF;
  regmap[10] = count >> 16;
  count = (uint32_t)(ot.state[5].period * OCTO_CLK_FREQ);
  regmap[11] = count & 0xFFFF;
  regmap[12] = count >> 16;
  count = (uint32_t)(ot.state[6].period * OCTO_CLK_FREQ);
...
void Octopus::EnableTimer(Pin pin, bool enable) {
  if (pin < OUT1_BOTTOM || pin > OUT8_TOP) {
    printf("EnableTimer: pin (%d) out of range.\n", (int)pin);
    return;
  }
  ModReg(TIMER_CONTROL, 0, 1, pin - 8, 1);
}

void Octopus::ConfigureAOM(const AOM& oa) {
  assert(oa.channel >= 1);
  assert(oa.channel <= 4);
