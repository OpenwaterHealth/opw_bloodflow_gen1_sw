...
void ExecNode::AddProducer(ExecNode* producer) {
  producer->consumers_.push_back(this);
  producer->up_.push_back(0);

  this->producers_.push_back(producer);
  this->down_.push_back(0);

  if (producer->down_queue_.size() > this->down_queue_.size()) {
    this->resize(producer->down_queue_.size());
  } else {
    producer->resize(this->down_queue_.size());
  }
}
