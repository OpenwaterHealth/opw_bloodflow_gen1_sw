...
class ExecNode {
 public:
  ...
  // Manually send data to all consumers of this node, as if this node had produced the data itself.
  // All consumer nodes will execute in a separate thread. In practice, this is only called by the
  // root node, everything downstream of the root being handled by Schedule().
  // @note will NOT run this node's Exec() function before sending downstream to consumers
 ...
 protected:
  ...
  virtual bool Schedule(void* data, ExecNode* producer);

  // Run upstream node cleanup
  virtual void CleanUp(void* data, ExecNode* consumer);
  ...
  class ThreadManager {
   public:
    ...
    // Schedule a node to be run.
    void Schedule(ExecNode* en, void* data);

    // Thread function: check for a node to be run, and run it.
    void Thread();

    // Set thread pool size (typically before start-up).
    void SetNumberThreads(size_t threads);

    // For testing only: peek into the scheduled nodes.
    CircularBuffer<std::pair<ExecNode*, void*>>& scheduledNodes() { return scheduled_nodes_; }
  ...
  std::mutex mutex_;
  std::mutex cleanup_;
  // Semaphore to alert sleeping ExecNode to check if it is next in line to be run.
