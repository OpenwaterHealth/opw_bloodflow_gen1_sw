...
std::vector<std::pair<uint16_t, int>> Driver::addresses_;

void Driver::Init(bool recheck) {
  if (!recheck && addresses_.size() > 0) return;
  addresses_.clear();
  ...
  for (int i = 0; i < usb.DeviceCount(); ++i) {
    ...
    // PID 0x4F10 is an unprogrammed gumstick, PID 0x4F11 is a programmed gumstick.
...
int Driver::DeviceNumber(int address) {
  CCyUSBDevice usb(NULL);
  int n = 0;
  while (true) {
...
int Driver::NumDevices(uint16_t pid, bool recheck) {
  Init(recheck);
...
int FX3::Reset() {
  ...
  // TODO(jfs) we can likely get rid of this magic sleep
  Component::SleepMs(5000);

  // make sure the bootloader is running after reset
...
int FX3::CmdRead(uint8_t req, uint16_t wValue, uint16_t wIndex, uint16_t len,
  ...
  ctrl_->Index = wIndex;
  long buflen = len;
  ctrl_->XferData(buf, buflen);  // TODO: This can fail, but presumably sets buflen to 0?
