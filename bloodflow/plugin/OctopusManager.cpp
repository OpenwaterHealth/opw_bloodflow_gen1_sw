...
bool OctopusManager::init(const json& systemParameters) {
  int code = octopus_->Open();
  if (code != 0) {
    std::cerr << "ERROR opening octopus: " << code << std::endl;
    return false;
  }
  std::cout << "Opened octopus " << octopus_->SerialNumber() << std::endl;
  return InitializeOctopus(systemParameters);
}

void OctopusManager::TriggerDataCollection() {
...
bool OctopusManager::EnableSystemChannels(bool channelON) {
  if (channelON) {
    ...
    octopus_->EnableTimer(aomPulseChopGate_.pin, channelON);
  } else {
    for (const systemTimer& t : timers_cameraFrameValid_) {
    ...
    DisableOutput(aomPulseChopGate_.timer, aomPulseChopGate_.pin);
  }
  return true;
}

bool OctopusManager::ConfigureSystemChannels() {
  ...
  octopus_->ConfigureTimer(aomPulseChopGate_.timer);
  return true;
}

bool OctopusManager::SetDataCollectionTriggerPin(Octopus::Pin triggerPin) {
...
bool OctopusManager::SetAOMPulsewidth(
      int numImages, double delay, double width, double freq_Hz, double volt_V) {
  Octopus::Timer& timer_AOMpulseChopGate = aomPulseChopGate_.timer;
  memset(&timer_AOMpulseChopGate, 0, sizeof(Octopus::Timer));
  ...
  timer_AOMpulseChopGate.state[1].period = delay; // Delay before trigger to pulse chopping AOM
  ...
  timer_AOMpulseChopGate.state_transition_count = 3 * (numImages + 1) + 1;  // (num states * (num bright/dark pairs + 1 for FSIN init) + 1 for state 0
      // ToDo(jfs): -1 option, for infinite?
  timer_AOMpulseChopGate.out_select = pin_AOMpulseChopGate_;
  octopus_->ConfigureTimer(timer_AOMpulseChopGate);
  aomPulseChopGate_.pin = pin_AOMpulseChopGate_;
  octopus_->EnableTimer(pin_AOMpulseChopGate_, true);
...
bool OctopusManager::InitializeOctopus(const json& systemParameters) {
  ////////// System Dependent Delay Variables //////////
  const double TTLPulseWidth_s = 1e-6;
  const double mhz2hz = 1000000.0;
  ...
  double triggerOffset_s = systemParameters["delayParameters"]["triggerOffset_s"].get<double>();
  ...
  double pulseWidth_s = systemParameters["delayParameters"]["pulseWidth_s"].get<double>();
  double chEDelay_s = systemParameters["delayParameters"]["chEDelay_s"].get<double>();  // Pulse chopping AOM in pseudo-pulsed systems
  hwTriggered_ = systemParameters["hardwareParameters"]["hwTrigger"].get<int>();  // Is system triggered by TTL button or sw trig

  double aomChopDelay_s = chEDelay_s + (1.0 / frameAcquisitionRate_Hz);  // 1 dark image plus regular delay to chopper
  ...
  // For hwTrigger, waits in state 0; state 1 is necessary for something to look for the trigger.
  // States 2 & 3 bypass state 1 so as not to require a trigger for each frame.
  ...
  while (dataCollectionGatePeriod_s > maxOctopusPeriod) {
    ...
    if (++idx >= swGatePeriods.size()) {
      printf("ERROR: Octopus gate period too long (%g sec max).\n", 5 * maxOctopusPeriod);
      return false;
    }
  }
  ...
  // Always expect four cameras. Harmless if fewer; testing singly much easier w/o cable swapping.
  int numCameras = 4;
  ...
  for (int i = 0; i < numCameras; i++) {
    ...
    systemTimer tempFrameValidTimer = { timer_cameraFrameValid, pins_cameraFrameValid_[i] };
    timers_cameraFrameValid_.push_back(tempFrameValidTimer);
  }
  ...
  for (int i = 0; i < numCameras; i++) {
    ...
    timer_cameraFSIN.state_transition_count = 3 * (2 * (numImages + 1)) + 1;  // ToDo(jfs): -1?
    timer_cameraFSIN.out_select = pins_cameraFSIN_[i];
    octopus_->ConfigureTimer(timer_cameraFSIN);
    systemTimer tempSystemTimer = { timer_cameraFSIN, pins_cameraFSIN_[i] };
    timers_cameraFSIN_.push_back(tempSystemTimer);
  }

  // ToDo(jfs): This is marked as bool but never returns false.
  SetAOMPulsewidth(numImages, aomChopDelay_s, pulseWidth_s, AOM4Freq_Hz, AOM4Volt_V);
...
bool OctopusManager::EnableFrameValid(bool validHIGH) {
  for (const systemTimer& t : timers_cameraFrameValid_) {
    if (validHIGH) {
